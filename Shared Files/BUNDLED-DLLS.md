...
## EnhancedFlowDocumentControls — vendored as a local project

- Was: `EnhancedFlowDocumentControls` 1.0.0 (Tony Hallett, MIT) — the enhanced FlowDocument reader +
  find toolbar behind FCC's read-me tool window.
- Resolution: `ThirdParty/EnhancedFlowDocumentControls/` (see its `VENDORED.md`). Retargeted to net472,
  referenced by `FineCodeCoverage2022` via `ProjectReference`. Its own dependency on **Fasterflect**
  was dropped — `ViewModel/FindToolBarReflector.cs` was rewritten to use plain `System.Reflection`.

## Markdig.Wpf — vendored as a local project

- Was: `Markdig.Wpf` 0.5.0.1 (Nicolas Musset, MIT, abandoned) — renders Markdig Markdown to a WPF
  FlowDocument; `MarkdigExtended` subclasses its renderers.
- Resolution: `ThirdParty/Markdig.Wpf/` (see its `VENDORED.md`). Multi-targets
  `net462;netcoreapp3.1;net5.0-windows` to match `MarkdigExtended`, references the solution's existing
  `Markdig` (not its original 0.22.0 pin). Referenced via `ProjectReference`; `PackageReference` + CPM
  entry removed.

All of the above: full solution builds clean (0 warnings) and all 808 tests pass (807 + 1 skipped).
Note: the WPF rendering itself (read-me tool window, options pages, find toolbar) is not covered by the
unit tests, so it is worth a manual smoke-test in a VS experimental instance before release.
