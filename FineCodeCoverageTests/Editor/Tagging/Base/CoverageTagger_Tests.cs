...
namespace FineCodeCoverageTests.Editor.Tagging.Base
{
    internal class CoverageTagger_Tests
    {
        ...
        public void Should_Raise_Tags_Changed_For_CurrentSnapshot_Range_When_CoverageChangedMessage_Applies_No_Line_Numbers(bool appliesTo)
        {
            ...
            coverageTagger.Handle(new CoverageChangedMessage(appliesTo ? "filepath" : "otherfile", null));
        ...
        public void Should_Raise_Tags_Changed_For_Containing_Range_When_CoverageChangedMessage_With_Line_Numbers()
        {
            ...
            coverageTagger.Handle(new CoverageChangedMessage("filepath", new List<int> { 1, 2, 3}));
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void Should_HasCoverage_When_BufferLineCoverage_HasCoverage(bool hasCoverage)
        {
            var mockBufferLineCoverage = new Mock<IBufferLineCoverage>();
            mockBufferLineCoverage.SetupGet(bufferLineCoverage => bufferLineCoverage.HasCoverage).Returns(hasCoverage);
            var coverageTagger = new CoverageTagger<DummyTag>(
                new Mock<ITextInfo>().Object,
                mockBufferLineCoverage.Object,
        ...
        {
            var autoMoqer = new AutoMoqer();
            var mockBufferLineCoverage = autoMoqer.GetMock<IBufferLineCoverage>();
            mockBufferLineCoverage.SetupGet(bufferLineCoverage => bufferLineCoverage.HasCoverage).Returns(true);
        ...
        public void Should_Not_Raise_TagsChanged_For_CoverageTypeFilterChangedMessage_If_No_Coverage()
        {
            var coverageTagger = new CoverageTagger<DummyTag>(
                new Mock<ITextInfo>().Object,
                new Mock<IBufferLineCoverage>().Object,
        ...
        public void Should_Return_No_Tags_If_ICoverageTypeFilter_Is_Disabled()
        {
            ...
            autoMoqer.SetInstance(new Mock<IDynamicLineAndSnapshotSpansLogic>(MockBehavior.Strict).Object);
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void Should_GetLineSpans_From_DynamicLineAndSnapshotSpansLogic_For_The_Spans_When_Coverage_And_Coverage_Filter_Enabled(bool newCoverage)
        {
            var autoMoqer = new AutoMoqer();
            var mockBufferLineCoverage = autoMoqer.GetMock<IBufferLineCoverage>();
            mockBufferLineCoverage.SetupGet(bufferLineCoverage => bufferLineCoverage.HasCoverage).Returns(true);
            ...
            if (newCoverage)
            {
                coverageTagger.Handle(new CoverageChangedMessage("filepath",null));
            }

            coverageTagger.GetTags(spans);

            autoMoqer.Verify<IDynamicLineAndSnapshotSpansLogic>(dynamicLineAndSnapshotSpansLogic => dynamicLineAndSnapshotSpansLogic.Apply(
               mockBufferLineCoverage.Object, spans));
        }

        [Test]
        public void Should_GetTagsSpans_For_Filtered_DynamicLineAndSnapshotSpans()
        {
            var autoMoqer = new AutoMoqer();
            var mockBufferLineCoverage = autoMoqer.GetMock<IBufferLineCoverage>();
            mockBufferLineCoverage.SetupGet(bufferLineCoverage => bufferLineCoverage.HasCoverage).Returns(true);
            ...
            var dynamicLineAndSnapshotSpans = new List<IDynamicLineAndSnapshotSpan>
            {
            ...
            var expectedDynamicLineAndSnapshotSpan = dynamicLineAndSnapshotSpans[4];

            var mockLineSpanTagger = autoMoqer.GetMock<ILineSpanTagger<DummyTag>>();
            var tagSpan = new TagSpan<DummyTag>(expectedDynamicLineAndSnapshotSpan.Span, new DummyTag());
            mockLineSpanTagger.Setup(lineSpanTagger => lineSpanTagger.GetTagSpan(expectedDynamicLineAndSnapshotSpan)).Returns(tagSpan);

            autoMoqer.Setup<IDynamicLineAndSnapshotSpansLogic, IEnumerable<IDynamicLineAndSnapshotSpan>>(
                dynamicLineAndSnapshotSpansLogic => dynamicLineAndSnapshotSpansLogic.Apply(
                    It.IsAny<IBufferLineCoverage>(),
                    It.IsAny<NormalizedSnapshotSpanCollection>()
                    )
                )
                .Returns(dynamicLineAndSnapshotSpans);
        ...
        public void Should_Have_No_Tags_When_FileIndicatorVisibility_Is_Initially_False(bool initiallyVisible)
        {
            ...
            autoMoqer.Setup<IDynamicLineAndSnapshotSpansLogic, IEnumerable<IDynamicLineAndSnapshotSpan>>(dynamicLineAndSnapshotSpansLogic =>
                dynamicLineAndSnapshotSpansLogic.Apply(It.IsAny<IBufferLineCoverage>(), It.IsAny<NormalizedSnapshotSpanCollection>())
            ).Returns(new List<IDynamicLineAndSnapshotSpan> { mockLineSpan.Object });
